...
| 5 · overnight | `.\run_overnight.ps1` — groq + mistral, one provider at a time, resume-safe. Gemini stays off unless `-IncludeGemini`. Does **not** start Cohere or 7×1000. `.\run_overnight.ps1 -Smoke` is one trial each for the other models (`llama-3.1-8b-instant`, `llama-3.3-70b-versatile`) under `results\probes\`, not the 996. |
