...
| Phase | Command |
|-------|---------|
| Pilot (18 trials) | `python run_grid.py --provider groq --pilot` → `results/pilot_groq_allam-2-7b.jsonl` (18 lines) |
...
| 5 · full grid | `python run_grid.py --provider gemini --seeds 7` · `--provider mistral --seeds 7` · `--provider cohere --subset priority --seeds 8` · `--provider groq --seeds 7` (all resumable; re-run the same command after a quota wall) |
| 6 · full stats | `python stats.py "results/*.jsonl" --boot 2000 --perms 5000 --kappa labeling/kappa.json --json results/summary.json` |
...
| File | Role |
|------|------|
| `providers.py` | `BaseProvider.generate(history, variant) -> (json, out_tokens, latency_ms)`; `RateLimiter`; `CallLedger` (`.quota/`); validating `extract_last_json`; `REGISTRY` of free-tier defaults |
