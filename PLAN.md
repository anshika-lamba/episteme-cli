...
| Model wraps JSON in prose, fences, or a trailing comma | strict `json.loads` only. Failure is a formatting collapse: `parse_fail=true`, `rel=null`. Not repaired, not imputed as 0.0, not filled from the next valid command. Two in a row end the trial; the grid continues | report `n_anomaly_eval_collapsed` and the parse-failure rate. Do not treat null as exact-zero |
