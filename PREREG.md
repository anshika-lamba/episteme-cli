...
* 2026-09-23 — `.\run_overnight.ps1 -Smoke` runs one probe trial (`--max-trials 1`) for the other catalog models and writes `results/probes/`. Those files are not part of the 996 and must not be pooled into confirmatory stats. Gemini and Cohere stay excluded unless `-IncludeGemini` / `-IncludeCohere`.
* 2026-09-23 — On this Windows host `wsl --exec bash -c` returned 1 for a deliberate `exit 17`. The harness now tries `bash`, `/bin/bash`, `sh`, and `/bin/sh`, with and without `--exec`. It accepts a form only if the script's stdout shows it ran. If `exit 17` comes back nonzero but not 17, and `exit 0` stays 0, success checks use zero versus nonzero and the Linux `$?` is written beside the trial. A form that turns `exit 17` into 0 is still refused.
* 2026-09-23 — The dedicated trial user `episteme` is withdrawn. `wsl -u episteme` exits 127 when that account does not exist, and creating the account mutates the distro. Trials run as the default WSL user. A bare `sudo` in the trial script is a shell function that returns 127. That function does not intercept `/usr/bin/sudo`. JSON parsing and resume are unchanged.
